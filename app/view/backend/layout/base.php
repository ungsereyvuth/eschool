...
if(is_array($pagename)){
	...
	$pagetitle = 'Untitled';$pagename = strtolower($pagename);
	foreach($layout_cate as $cate_value){
		$pagetitle = isset($pageData->label->$cate_value['title']->$pagename->title)?$pageData->label->$cate_value['title']->$pagename->title:'Untitled';
		if(isset($pageData->label->$cate_value['title']->$pagename->title)){break;}
	}
}
//---- end page variables
...
}elseif($pageData->logoutRequired and $usersession->isLogin()){header("location: /");}
...
if(count($pageArr)){
	foreach($pageArr as $key=>$value){
		if($key==(count($pageArr)-1)){
			if(is_array($value)){
				$breadcrumb_txt .= '<li class="active tooltips" title="'.$value['title'].'">'.((mb_strlen($value['title'],"utf-8")>20)?(mb_substr($value['title'],0,20,"utf-8").'...'):$value['title']).'</li>';
			}else{
				$bc_txt = 'Untitled';
				foreach($layout_cate as $cate_value){
					$bc_txt = isset($pageData->label->$cate_value['title']->$value->title)?$pageData->label->$cate_value['title']->$value->title:'Untitled';
					if(isset($pageData->label->$cate_value['title']->$value->title)){break;}
				}	
			...
			if(is_array($value)){
				...
				foreach($layout_cate as $cate_value){
					$bc_url = '/'.$selected_language.(isset($pageData->label->$cate_value['title']->$value->url)?$pageData->label->$cate_value['title']->$value->url:'Untitled');
					$bc_txt = isset($pageData->label->$cate_value['title']->$value->title)?$pageData->label->$cate_value['title']->$value->title:'Untitled';
					if(isset($pageData->label->$cate_value['title']->$value->title)){break;}
				}				
