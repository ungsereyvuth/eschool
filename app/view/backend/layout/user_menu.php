			...
			foreach($pageData->data->component->user_menu as $key=>$value){
				if(isset($value['child']) and is_array($value['child'])){
					...
					foreach($value['child'] as $sub_key=>$sub_value){
						...
						$sub_menu.='<li class="'.$active_class.'"><a href="/'.$pageData->lang->selected.$sub_value['url'].'" title="'.$sub_value['title'].'"><span class="menu-item-parent">'.$sub_value['icon'].' '.$sub_value['title'].'</span><span id="menu_'.$sub_value['code'].'"></span></a></li>';
					}

					echo '<li class="'.($sub_active?'open':'').'"><a href="#" title="'.$value['title'].'">'.$value['icon'].' <span class="menu-item-parent">'.$value['title'].' <span id="menu_'.$value['code'].'"></span></span></a>
								<ul id="menu_'.$value['id'].'" style="'.($sub_active?'display: block;':'').'">'.$sub_menu.'</ul>

						<li>';

				}else{
					echo '<li class="'.($active_page_id==$value['page_id']?'active':'').'">
