			...
			if(is_numeric($recordid) and $recordid>=0){
				...
				elseif($recordid<>$usersession->info()->id and !isAdmin($usersession->info()->id)){
					...
					if(!filter_var($reg_fields['text']['username'], FILTER_VALIDATE_EMAIL)){
						$exist = $qry->qry_count("SELECT id FROM users where username='".$reg_fields['text']['username']."' and id <> $recordid limit 1");
						if($exist){$err_fields[]=array('name'=>'username','msg'=>$layout_label->message->username_exist->title);}
