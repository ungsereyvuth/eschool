...
function khmerDate($date,$format='full'){
	...
	$khdate = array('d'=>enNum_khNum($d),'m'=>$khmerMonth[$m-1],'y'=>enNum_khNum($y),'full'=>$full,'full_dt'=>$full_dt);
...
function content($code){
	...
	$content = array();
	$contentData = $qry->qry_assoc("select c.*,if(t.title_t='' or t.title_t is NULL,c.title,t.title_t) title,if(t.description_t='' or t.description_t is NULL,c.description,t.description_t) description from content c 
