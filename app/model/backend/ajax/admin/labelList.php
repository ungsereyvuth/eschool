	...
	public function data($data){
		...
		foreach($rowData as $key=>$value){	
			...
			if(is_numeric($qryData->lang_id)){
				$translation_btn = '<button class="load_form_btn btn btn-xs rounded btn-default" type="button" data-cmd="'.$load_data_cmd['load_translation'].'" data-target_formid="newPageLabelTranslation" data-recordid="'.encodeString($value['id'].'_'.$qryData->lang_id,$encryptKey).'"><i class="fa fa-language"></i> Translate</button>';
			...
			$dataListString .= '<tr>
									...
									<td class="txtCenter">
										<button class="load_form_btn btn btn-xs rounded btn-default v_mgn5" type="button" data-cmd="'.$load_data_cmd['load_label'].'" data-target_formid="newPageLabel" data-recordid="'.encodeString($value['id'],$encryptKey).'"><i class="fa fa-pencil"></i> Edit</button>
										'.$translation_btn.'
									</td>								
								</tr>';
