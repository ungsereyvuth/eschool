...
class admin_userlist{
	public function data($data){
		...
		if($qryData->txt_search<>''){$sql_condition.=" and (u.fullname_kh like '%$qryData->txt_search%' or u.fullname_en like '%$qryData->txt_search%' or u.email like '%$qryData->txt_search%' or u.username like '%$qryData->txt_search%')";}
		...
		foreach($rowData as $key=>$value){	
			...
			$dataListString .= '<tr>
									...
									<td class="txtLeft">
										<div>
											<span class="tooltips" title="User ID: '.$value['id'].'">'.$value['fullname_kh'].' '.$status.'</span>											
										</div>
